...
const UCTableHeader: React.FC<THeadProps> = ({ children, className, ...rest }: THeadProps) => {
    return <thead
        {...rest}
        className={clsx("bg-blue-900/60 dark:bg-teal-500/60 text-center text-xs font-medium titlecase tracking-wider text-gray-200 dark:text-gray-900 antialias",className)}>
        {children}
    </thead>
}
